...
- **주요 경험:**
  ...
  - **Android 5~16 · OEM 4사** — 삼성·LG·샤오미·구글 등 제조사마다 다른 API 동작을 Reflection 호환성 레이어에서 흡수.
  - **멀티모듈 · Java→Kotlin** — 기능 수정 시 다른 모듈까지 영향 → `core / detection / ui` 분리, API 시그니처 유지. 모듈 단위 테스트·기능 선택 연동.
  - **업데이트 안정성** — 패턴 갱신마다 전체 파일 다운로드 → 패턴/엔진 분리로 필요한 파일만 갱신. 실패 시 재시도·무결성 재검증·중복 실행 방지, WorkManager + Coroutines로 백그라운드 작업 정리.
  - **JNI 메모리 릭** — 원인 추적·해결.
...
- **주요 경험:**
  - **실기기 검증** — TFLite 양자화 모델 적용. 화면 캡처 **720p·30fps 미달**, 추론 지연도 실시간 기준 초과.
  - **구조 전환** — FPS·추론 지연 측정값 공유 → 온디바이스 대신 클라우드 비동기 구조로 전환 결정.
  - **서비스 출시** — 영상 업로드·URL 입력·결과/랭킹 흐름 구현 → **50만+ 다운로드 앱**에 출시.
  - **모델 개발 서브** — 자체 데이터셋 구축 후 모델 튜닝 → 동양인 얼굴 영상의 딥페이크 탐지 정확도 **65% → 95%**(내부 검증셋).
...
- **개요:** 서버 없이 Android/iOS 단말에서 딥페이크를 탐지하는 SDK. Android/iOS가 같은 전처리·추론 경로를 쓰게 하고, **SDK 재배포 없이 모델만 교체**할 수 있도록 C++ 공통 코어 기반으로 설계·개발.
- **주요 경험:**
  - **크로스플랫폼 · Android + iOS** — Kotlin/Swift 인터페이스 → C++ 공통 코어 → ONNX Runtime 추론 계층으로 분리. 탐지·전처리·후처리를 C++에서 공통화.
  - **모델 교체·배포** — 설정 + 암호화 모델 + 서명을 패키지 하나로 분리 → SDK 재배포 없이 모델 교체. mVaccine 서명 형식 계승 + AES-256-GCM 봉투 암호화 자체 설계, 전체 SHA-256 검증 후 스테이징 교체. 검증 실패 시 기존 설치본 유지, 로드 실패 시 재설치 복구.
  - **실앱 OTA 검증** — 고객사 배포 전 사내 앱(라온 모바일 시큐리티)에 연동해 모델 교체 검증. 재시작·오프라인에서도 업데이트된 모델 유지 확인.
  - **모델 최초 설치 · 14.8초 → 0.3초** — 설치 전 검증과 로드에서 두 번 하던 복호화를 한 번으로, OTA 모델 검증은 유지.
  - **모델 단위 API 재설계** — 전역 설정으로는 다중 모델 불가 → **모델 1개 = 인스턴스 1개** 구조. 전·후처리 Builder 주입, 갱신은 앱에서 제어.
  - **단말·런타임 호환성** — NNAPI 벤더 드라이버 크래시 확인 → Android는 XNNPACK 우선. iOS는 CoreML 우선 구성. ARM32 SIGBUS는 ORT 그래프 최적화 단계까지 추적해 32비트에서 문제 최적화 단계만 비활성화. Android 15+ 16KB 페이지 크기 대응.
  - **출력 재현·검증** — 협력사 참조 구현 대비 출력 오차 **≤ 1e-5**(얼굴 검출 박스 고정). C++ 회귀 **257건** · 실단말 테스트 **70+건** · iOS XCTest **130+건**.
  - **단말 간 판정 차이 해결** — 단말마다 오디오 디코딩 결과가 달라짐 → MediaParser 전환, 3개 단말의 오디오 데이터(PCM) 일치 확인.
  - **음성·영상 전처리 공통화** — 멜 스펙트로그램·컬러맵·리샘플링·몽타주 전처리를 Python에서 C++ 공통 코어로 이식.
...
- **200+ 기업 · 월간 보고서 · 기존 수작업 2~3일** — 리서치→작성→검증→수정을 단계별 에이전트로 자동화. 검증 실패 항목만 재생성.

#### 악성앱 탐지 AI
- **역할·기간·기술:** 개발 리드(모델 포함) · 2026.02 ~ 2026.08 · Python · PyTorch · Drebin · APIGraph · DexRay · DetectBERT · MaMaDroid · MalScan
- **모델 선정** — Drebin·APIGraph·DexRay를 비교 검증해 최종 모델로 Drebin 선정.
- **오탐 개선** — 구형 앱 오탐이 높아 2012~2017년 악성앱 1.8만 건 수집·재학습. 최신 악성앱 탐지율 **98.37%** 유지, 구형 앱 오탐률 **32.18% → 10.75%**, 신형 앱 **1.16% → 1.00%**.
- **평가 방식** — 초기 DexRay 평가의 랜덤 분할 F1 0.925가 과대평가임을 확인 → 수집 시점 기준 평가로 전환.
...
- **Recall 99.34% / 정상 통화 오탐 22%** — 제품 기준 미충족으로 출시 보류 제안.
...
- **온디바이스 AI** — 사진첩의 유사 셀카 그룹핑·베스트샷 추천. ML Kit 얼굴 검출 + 자체 로직, 임계값별 정확도–오탐률 측정·튜닝.
- **1인 개발·운영** — 멀티모듈 · AdMob 광고 연동·수익화 · Play Store 배포·업데이트.
