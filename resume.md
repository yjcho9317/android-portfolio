...
Android SDK · NDK / JNI · C++ · CMake · AIDL / IPC · 멀티모듈(Multi-module) · Public API 설계 · 하위 호환성(Backward Compatibility) · DX(개발자 경험) · OEM 단편화 대응 · ProGuard / R8
...
**AI / ML 적용**
TFLite · PyTorch · ML Kit · MediaProjection · Model Quantization · Knowledge Distillation · KoBERT · NLP · GNN · MLflow · FastAPI · Docker · Python

**iOS · Cross-platform**
iOS SDK · Swift · Objective-C · TypeScript · MCP Protocol

**Quality · DevOps**
JUnit · Frida · Jenkins · GitHub Actions · Firebase Crashlytics / Analytics · Git · npm
...
- **기간:** 2019.04 ~ 현재
- **기술 스택:** Android SDK · NDK · iOS SDK · Kotlin · Coroutines · Java · C++ · Objective-C · Swift · MVVM · Clean Architecture · Multi-module · WorkManager · ProGuard / R8 · JUnit · Firebase Crashlytics/Analytics · Jenkins
...
- **핵심 성과:** Public API 설계·운영(호스트 앱 코드 수정 0건 원칙, 개발자 경험 운영), 멀티모듈(core/detection/ui) 분리 + Java→Kotlin 마이그레이션, SDK 사이즈 최적화(통합→분할 다운로드, 패턴/엔진 파일 분리), 200+ 고객사 OEM 호환성 대응(Android 5~16, 삼성·LG·샤오미·Pixel, Reflection 기반 호환성 레이어), AIDL/IPC + WorkManager + Coroutines 기반 비동기 처리, JNI 메모리 릭 추적·해결, KISA 백신 인증 획득, JUnit + Jenkins CI/CD + Firebase 기반 품질 체계, iOS 탈옥/원격/디버깅 탐지 모듈 개발.
...
- **역할:** 개발 및 운영
- **기간:** 2021.07 ~ 현재
- **기술 스택:** Kotlin · Coroutines + Flow · Room · Hilt · MVVM · Lifecycle · Navigation · Paging · DataStore · Retrofit2 · OkHttp3 · Firebase Crashlytics/Analytics
...
#### 3. 클라우드 서비스 전환 (mVaccine SDK 비즈니스 모델 변경)

- **소속:** 라온시큐어
- **역할:** 앱 개발 리드 (5인 팀)
- **기간:** 2025.02 ~ 2025.10
- **기술 스택:** Android SDK · Java · Kotlin
- **개요:** mVaccine SDK의 비즈니스 모델을 라이선스 기반에서 사용량 과금으로 전환.
- **핵심 성과:** 라이선스 기반 모델의 한계 진단(로컬 DB 용량 제약으로 탐지 패턴 확장 한계, 실 사용 데이터 수집 불가, CDN 패턴 배포 비용 누적) → 사용량 과금 + 클라우드 호출 기반으로 전환 결정·실행, 탐지 패턴 DB 5배+ 확장 가능, 실 사용 횟수 기반 집계·통계 수집 구조 확보, CDN 대비 서버 비용 절감.


#### 4. AI 딥페이크 탐지 - 온디바이스 vs 클라우드 아키텍처 판단

- **소속:** 라온시큐어
- **역할:** 앱 개발 리드 / 모델 개발 서브 (6인 팀)
- **기간:** 2024.04 ~ 2024.12
- **기술 스택:** Android SDK · Kotlin · Python · PyTorch · TFLite · MediaProjection · i3d · ResNet
...
#### 5. AI 보이스피싱 탐지 - Recall 99.34% / FP 22% 출시 보류 판단
...
#### 2. nworks - NAVER WORKS MCP Server
*1인 설계·개발, 오픈소스*

- **기술 스택:** TypeScript · MCP · npm · GitHub Actions
- **개요:** AI 에이전트가 NAVER WORKS(메일/캘린더/메시지)를 직접 호출할 수 있게 만든 CLI & MCP Server. **awesome-mcp-servers 등재, Glama AAA 인증, 다국어 README(EN/KO/JA).**
...
- **기술 스택:** TypeScript · npm · Security · GitHub Actions
...
- **탐지 엔진:** 양방향 스캐닝, SHA-256 기반 도구 설명 해시 피닝, 정책 엔진(allow/deny + 인자 검증), SQLite 감사 로그.
- **품질 체계:** 56개 탐지 패턴, 1,426개 테스트, 자체 측정 86% 탐지율. OWASP MCP Top 10 중 9개 항목 커버.
...
**신한대학교** · 컴퓨터공학 전공 · 2018.08 졸업 (편입, 학점 4.22 / 4.5)
- 졸업 프로젝트: 비콘 기반 스마트 홈 서비스 개발 · 학술 논문 대회 수상

**신흥대학교** · 웹프로그래밍 전공 · 2015.02 수료 (학점 3.95 / 4.5)
